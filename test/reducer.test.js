...
import reducer, { actions, actionTypes } from '../src/reducer'
  ...
  describe('actions', () => {
    describe('FETCH', () => {
      it('should create an action with object args', () => {
        ...
        const name = 'fake rpc'
        expect(actions.fetch({ name, args })).toEqual({
      ...
      it('should create an action with no args', () => {
        const name = 'fake rpc'
        expect(actions.fetch({ name })).toEqual({
          type: actionTypes.FETCH,
          name,
        })
      })
    ...
    describe('$action_FETCH_START', () => {
      it('should create an action', () => {
        ...
        const id = 1
        expect(actions.fetchStart({ name, args, id })).toEqual({
    ...
    describe('$action_FETCH_SUCCESS', () => {
      it('should create an action', () => {
        ...
        expect(actions.fetchSuccess({ name, args, id, result })).toEqual({
    ...
    describe('$action_FETCH_FAIL', () => {
      ...
      expect(actions.fetchFail({ name, args, id, error })).toEqual({
